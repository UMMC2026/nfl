...
# B2B penalties (already factored into matchup adjustments above for CLE/DAL)
B2B_TEAMS = ["CLE", "DAL"]
...
def main():
    ...
    for pick in data["picks"]:
        ...
        print(f"{status} {player} {stat} {line}+ [{prob_change}]")
        if matchup_reason:
            print(f"   → Matchup: {matchup_reason}")
        if blowout_note:
            print(f"   → {blowout_note}")
