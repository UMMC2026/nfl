"""
SOCCER DATA BACKUP SYSTEM
=========================
Backup management for Soccer data including:
- API-Football cache responses
- ESPN Soccer cache
- Player stats cache

Usage:
    python soccer/data/soccer_data_backup.py --backup      # Create backup
    python soccer/data/soccer_data_backup.py --list        # List backups
    python soccer/data/soccer_data_backup.py --restore ID  # Restore
"""
...
def create_backup(reason: str = "manual") -> Dict:
    """Create a full backup of Soccer data."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    backup_id = f"soccer_backup_{timestamp}"
    backup_path = BACKUP_DIR / backup_id
    backup_path.mkdir(parents=True, exist_ok=True)
    
    print(f"\n{'='*60}")
    print(f"  SOCCER DATA BACKUP")
...
def main():
    parser = argparse.ArgumentParser(description="Soccer Data Backup")
