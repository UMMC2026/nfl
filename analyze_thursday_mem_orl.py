#!/usr/bin/env python3
"""
THURSDAY MEM @ ORL COMPREHENSIVE ANALYSIS
All stat types with Monte Carlo + Bayesian + DeepSeek + Ollama
"""
...
# Import extended stats (Wednesday + Thursday)
from extended_stats_dict import PLAYER_STATS, calculate_combo_stats

# Team Defense/Offense for MEM @ ORL
TEAM_DEFENSE = {
    'ORL': {'rank': 3, 'rating': 106.8, 'scheme': 'Aggressive switching with length', 'strength': 'Perimeter closeouts'},
    'MEM': {'rank': 14, 'rating': 111.2, 'scheme': 'Help-heavy with rim protection', 'strength': 'Interior defense'},
}

TEAM_OFFENSE = {
    'ORL': {'rating': 111.5, 'pace': 98.2, 'style': 'Banchero creation with spacing'},
    'MEM': {'rating': 115.8, 'pace': 101.3, 'style': 'Transition-heavy, Bane/JJJ scoring'},
}

COACHING_STYLES = {
    'ORL': {'coach': 'Jamahl Mosley', 'style': 'Defensive identity, player development'},
    'MEM': {'coach': 'Taylor Jenkins', 'style': 'Pace-and-space, transition attack'},
}

MATCHUPS = {'MEM': 'ORL', 'ORL': 'MEM'}

# Load Thursday props
with open('nba_thursday_mem_orl.json', 'r') as f:
...
def get_stat_params(player, stat):
    """Get mean and sigma for any stat type"""
...
def get_deepseek_analysis(player, stat, line, direction, mu, sigma, mc_prob, bay_prob, player_team, opp_team):
    ...
    prompt = f"""Analyze this NBA prop bet for Thursday afternoon MEM @ ORL (1pm):
...
Provide 2-3 sentence analytical breakdown focusing on statistical edge and afternoon game context."""
...
def get_ollama_commentary(player, stat, player_team, opp_team):
    ...
    if stat == 'pra':
        stat_context = "all-around production"
    elif stat in ['pr', 'pa', 'ra']:
        stat_context = f"{stat.upper()} combo stat"
    
    prompt = f"""NBA coaching matchup (2-3 sentences max):

{player_team} ({player_coach}: {player_style})
vs
{opp_team} ({opp_coach}: {opp_style})

How does {player}'s {stat_context} benefit from this matchup in an afternoon game?"""
...
# Process all props
print("\n" + "="*100)
print("THURSDAY MEM @ ORL COMPREHENSIVE ANALYSIS - 1:00PM CST")
...
# Display top 25 WITH FULL AI COMMENTARY
...
output_file = f"outputs/THURSDAY_MEM_ORL_{timestamp}.txt"
os.makedirs("outputs", exist_ok=True)

with open(output_file, 'w', encoding='utf-8') as f:
    f.write("="*100 + "\n")
    f.write("THURSDAY MEM @ ORL COMPREHENSIVE ANALYSIS - 1:00PM CST\n")
    ...
    for i, r in enumerate(results[:25], 1):
        ...
        f.write(f"MATCHUP: {r['team']} vs {r['opp']}\n")
